...
public class AbstractAutoServiceRegistrationTests {
	...
	public void portsWork() {
		...
		then(this.autoRegistration.isRunning()).isTrue()
			.as("Lifecycle not running");
		then(this.autoRegistration.getServiceRegistry())
			.isInstanceOf(TestServiceRegistry.class).as("ServiceRegistry is wrong type");
		TestServiceRegistry serviceRegistry = (TestServiceRegistry) this.autoRegistration
			.getServiceRegistry();
		then(serviceRegistry.isRegistered()).isTrue()
			.as("Lifecycle not registered");
	...
	public void eventsFireTest() {
		...
		then(this.postEventListener.config.getServiceId())
			.isEqualTo("testRegistration2");
	}
