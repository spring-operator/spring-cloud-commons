...
public class InetUtilsTests {
	...
	public void testConvert() throws Exception {
		try (InetUtils utils = new InetUtils(new InetUtilsProperties())) {
			then(utils.convertAddress(InetAddress.getByName("localhost")))
				.isNotNull();
		}
	}
	...
	public void testIgnoreInterface() {
		...
		try (InetUtils inetUtils = new InetUtils(properties)) {
			...
			then(inetUtils.ignoreInterface("docker0")).isTrue()
				.as("docker0 not ignored");
			;
			then(inetUtils.ignoreInterface("vethAQI2QT"))
				.as("vethAQI2QT0 not ignored").isTrue();
			then(inetUtils.ignoreInterface("docker1")).as("docker1 ignored")
				.isFalse();
		}
	}
	...
	public void testDefaultIgnoreInterface() {
		try (InetUtils inetUtils = new InetUtils(new InetUtilsProperties())) {
			then(inetUtils.ignoreInterface("docker0")).as("docker0 ignored")
				.isFalse();
		}
	}
	...
	public void testSiteLocalAddresses() throws Exception {
		...
		try (InetUtils utils = new InetUtils(properties)) {
			...
			then(utils.isPreferredAddress(InetAddress.getByName("5.5.8.1")))
				.isFalse();
		}
	}
